//
import * as library from "../../../schema/v1/code/library.js";
...
import * as quest from "../../../schema/v1/code/questionnaire.js";
//
...
export abstract class app extends outlook.page {
    ...
    async register(): Promise<Array<string> | undefined> {
            ...
            //The minimum requirement are the roles teh user will play in this 
            //application
            //
    ...
    private get_subscription_data(): Array<quest.label> {
            ...
            this.user!.role_ids!.forEach((myrole, i) => {
                    ...
                    reg.push([this.dbase!.name, myrole, [i], "name", this.user!.name!]);
                    //
                    //Indicate that we need to  save a subscription record
                    reg.push([user_dbname, "subscription", [i], "is_valid", true]);
                    //
                    //Indicate that we need to save a player
                    reg.push([user_dbname, "player", [i], "is_valid", true]);
                    //
                    //Collect the user roles in this application
                    reg.push([user_dbname, "role", [i], "id", myrole]);
            });
            //
