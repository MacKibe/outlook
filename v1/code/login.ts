...
import * as schema from "../../../schema/v1/code/schema.js"
//
//Resolve the server method for backend communication
import * as server from "../../../schema/v1/code/server.js"
//
//Resolve referemce to the io
import * as io from "../../../schema/v1/code/io.js"
...
//
//The possible value keys collected from this page. These are used for type-checking
//the possible input indices
...
export class page extends popup<user> {
    ...
    async check():Promise<boolean> {
        ...
        const inputs = <{[key in keys]:schema.basic_value|Error}> this.check_inputs();
        //
    ...
    check_inputs():{[key in keys]:schema.basic_value|Error}{
        //
        //Checking an input value
        const get_checked_value=(name: string): string|null=>{
            //
            ...
            //
            //The radio button's value must be set; otherwise teh form is not well designed.
            if(radio.value === "") 
        ...
        const get_input_value = (id: string): string|null=>{
            //
        ...
        //
        return {
            name: get_input_value('name')?? new Error(`Name must be provided`),
            password:get_input_value('password')?? new Error(`Password must be provided`),
            operation_id:get_checked_value('operation_id')?? new Error(`Specidy the required operation`),
            provider_id:get_checked_value('provider_id')?? new Error(`Specify the provider`)
        }
    }
...
class outlook extends provider {
    ...
    //
    //This is our custom-made authentication method using php hashing. 
