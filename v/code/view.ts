...
export class view {
   ...
   public get_attribute_value(element:HTMLElement, name:string):string{
    ...
    //The function will return null fail if there is no input value.
    public get_input_value(id: string): string|null {
        //
    ...
    //Return null if there is no cehcked radio button
    public get_checked_value(name: string): string|null {
        //
        ...
        //
        //The radio button's value must be set. Is a sign a prooly designed form if not
        if(radio.value === "") 
