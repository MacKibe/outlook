...
export class simple_page extends popup {
    //
    constructor() {
        //
    ...
    async check() {
        ...
        const result = await this.check_credentials(credentials);
        //
        //If the user is erroneous then report it as an error
        if (result instanceof Error)
            this.report(true, result.message);
        //    
    ...
    check_inputs() {
        ...
        const operation_id = inputs.operation_id;
    }
    //Check the user credentials
    async check_credentials(c) {
        //
        //Define a user, to be completed via registration or authetication.
        let result;
        //
        //Create the outlook service provider
        const Provider = new outlook(c.username, c.password);
        //
        //If the user is a visitor then register him...
        if (c.operation_id === 'register')
